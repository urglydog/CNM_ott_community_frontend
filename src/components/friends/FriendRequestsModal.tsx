...
export default function FriendRequestsModal({
  ...
  const handleAccept = async (requestId: number) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Không thể đồng ý";
      addToast(msg, "error");
    } finally {
  ...
  const handleReject = async (requestId: number) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Không thể từ chối";
      addToast(msg, "error");
    } finally {
      setProcessingIds((prev) => {
        const next = new Set(prev);
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-white">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {requests.length > 0
                ? `${requests.length} người muốn kết bạn`
                  ...
                  >
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      <button
                        ...
                        className="w-8 h-8 rounded-full bg-blue-500 hover:bg-blue-600 text-white flex items-center justify-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                        title="Đồng ý"
                        aria-label={`Đồng ý kết bạn với ${req.sender_display_name}`}
                      >
                        {isProcessing ? (
                      ...
                      <button
                        ...
                        disabled={isProcessing}
                        className="w-8 h-8 rounded-full bg-gray-100 hover:bg-red-50 text-gray-400 hover:text-red-500 flex items-center justify-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                        title="Từ chối"
                        aria-label={`Từ chối kết bạn với ${req.sender_display_name}`}
                      >

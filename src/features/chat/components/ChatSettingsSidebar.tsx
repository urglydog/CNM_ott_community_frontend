...
export default function ChatSettingsSidebar({
  ...
  const { socket, emitSendMessage } = useSocket() || {};
  ...
  const originalName = selectedFriend?.friend_original_name || selectedFriend?.friend_username || friendName;
  ...
  useEffect(() => {
    ...
    setNicknameValue(selectedFriend?.nickname || "");
  ...
  }, [friendId, myId, isOpen, friendshipId]);
    ...
    try {
      ...
      const updatedFriends = friends.map(f => {
        if (f.friendshipId === friendshipId) {
          return {
            ...f,
            friend_display_name: nick || f.friend_original_name || f.friend_username,
            nickname: nick,
          };
        }
        return f;
      ...
      if (selectedFriend && selectedFriend.friendshipId === friendshipId) {
        setSelectedFriend({
          ...selectedFriend,
          friend_display_name: nick || selectedFriend.friend_original_name || selectedFriend.friend_username,
          nickname: nick,
        });
      }
          ...
          <div className="mt-2 bg-white border-y border-gray-200">
            {nicknameEditing ? (
              ...
              <ListItem
                ...
                label={selectedFriend.nickname ? `Gợi nhớ: ${selectedFriend.nickname}` : "Đổi tên gợi nhớ"}
                onClick={() => { setNicknameEditing(true); setNicknameValue(selectedFriend.nickname || ""); }}
              />
            )}
            <ListItem
